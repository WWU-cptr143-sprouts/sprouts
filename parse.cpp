...
int Game::parsemove(char *move, int *from, int *to, int *creates, int *near, bool *clockwise, std::list<int> *encap)
{
	...
	for (int i = 0; move[i] != '\0'; i++) //Loop until we find a null character.
	{
		...
		if ((move[i] >= '0' && move[i] <= '9') || ((*from) != -1 &&
            (*to) != -1 && (*creates) != -1 && (move[i] == '-' || move[i] == ',')))
		{
        ...
        //Now we check for control characters ( ()@![] ).
        else
        {
            ...
            //If we encounter ) then we know the last number if either creates or near.
            else if (move[i] == ')')
            {
				while (stk.size() > 0)
				{
            ...
            //If we have a @ we need to set creates.
            else if (move[i] == '@')
            {
				while (stk.size() > 0)
				{
            ...
            // If we find a [ we need to set to.
            else if (move[i] == '[' && (*to) == -1)
            {
				while (stk.size() > 0)
				{
            ...
            //If we have a ] then we need to deal with listing the encapsulated nodes.
            else if (move[i] == ']')
            {
				while (stk.size() > 0) //Loop until the stack is empty.
				{
					...
					if (stk.size() > 0) //If we emptied the stack we can't use stk.top().
					{
                        ...
                        { 
                            ...
                            //If we had a range before the comma.
                            else
                            {
                ...
                //Ok, there was a digit at the end of the list [1-4,7] or such.
                else if (z != 0)
                { 
					(*encap).push_back(z-1); //Put it on the list.
				}
			}
            //Check if we found a ! AFTER to (from and creates are already set)
            else if (move[i] == '!' && (*from) != -1 && (*creates) != -1)
            { 
...
{
	static const char errors[8][128]= {
