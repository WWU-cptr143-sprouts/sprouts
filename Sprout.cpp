...
bool Sprout::available(int x, int y, int from, int to, Sprout ** sprouts)
{
  ...
  while (sprouts[i] != NULL)
  {
    ...
    //The point isn't on top of another, but we want to check if lines collide.
    else if(from != -1 && to != -1)
    {
      ...
      for (int j = 0; j < 4 * parent->count; j++) //Now we check all the Beziers...
      {
        for (int k = 0; k < 4 * parent->count; k++)
        {
          ...
          if (parent->connection[j][k] != NULL) //If we found a bezier then we need to see if there is a collision.
          {
