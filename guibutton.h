...
struct GUIButton : public GUIElement
{
    ...
    GUIButton(function<void()> pressEvent, wstring title, float minX, float maxX, float minY, float maxY, Color buttonColor = Color::V(0.75), Color textColor = Color::V(0), Color selectedButtonColor = Color::V(0.75), Color selectedTextColor = Color::RGB(0, 0, 1))
        : GUIElement(minX, maxX, minY, maxY), title(title), textColor(textColor), selectedTextColor(selectedTextColor), buttonColor(buttonColor), selectedButtonColor(selectedButtonColor), buttonPressEvent(pressEvent)
    {
    }
protected:
    GUIButton(wstring title, float minX, float maxX, float minY, float maxY, Color buttonColor = Color::V(0.75), Color textColor = Color::V(0), Color selectedButtonColor = Color::V(0.75), Color selectedTextColor = Color::RGB(0, 0, 1))
        : GUIElement(minX, maxX, minY, maxY), title(title), textColor(textColor), selectedTextColor(selectedTextColor), buttonColor(buttonColor), selectedButtonColor(selectedButtonColor), buttonPressEvent([](){throw logic_error("button press event function called instead of overridden member");})
    {
    }
private:
...
protected:
    ...
    virtual Mesh render(float minZ, float maxZ, bool hasFocus) const override
    {
        ...
        Mesh retval = (Mesh)transform(buttonTransform.concat(Matrix::scale(interpolate(2.0 / 3, minZ, maxZ))), diffuse);
        if(textHeight > 0 && textWidth > 0)
        {
            ...
            retval->add(transform(Matrix::scale(textScale).concat(Matrix::translate(xOffset, yOffset, -1).concat(Matrix::scale(minZ))), Text::mesh(title, textColor)));
        }
        retval->add(transform(buttonTransform.concat(Matrix::scale(interpolate(1.0 / 3, minZ, maxZ))), specular));
        return retval;
    }
    virtual void reset()
    {
